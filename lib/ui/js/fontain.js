...
// INIT ==============================================================================
  ...
  e.preventDefault(); 
    ...
    $(".accordion-section-content").addClass('zu'); // FALSCH HIER
    hh = $('input.fontsize').val();
    //console.log(hh);
    $('.accordion-section-content.zu input.fontdemo').css("margin-top", "" +-(hh/2)+"px");           // NEU
  }
...
//  $('input.fontdemo').css("margin-top", "-60px"); // PROBLEM
//  $('.accordion-section-content').css({           // PROBLEM
//      "height": "24px",                           // PROBLEM
//      "margin-top": "-24px"                       // PROBLEM
//  });                                             // PROBLEM
...
//  }
    ...
    if ($('a.fontdemo-showmore').length)  {
      $('a.fontdemo-showmore').click(function(e) {
      if ( $('.excluded:first').css("display") == "none" ) {
       $('.excluded').css("display", "inline");
       $(e.target).html("show less");
      }
      else if ( $('.excluded:first').css("display") == "inline" ) {
       $('.excluded').css("display", "none");
       $(e.target).html(showmoreButton);
      }
    // $( "#sortable" ).sortable( "refreshPositions" );
...
function initFlowVariants() {
  if ($('input.flowtext-fontsize').length && !isListPage)  {
    ...
    $('#invert').on("click", function(e) {
      if($(".flowtext").hasClass("positiv")) {
        $(".flowtext").removeClass("positiv");
        $(".flowtext").addClass("negativ");
        $("#invert").removeClass("positiv");
        $("#invert").addClass("negativ");
      } else {
        $(".flowtext").removeClass("negativ");
        $(".flowtext").addClass("positiv"); 
        $("#invert").removeClass("negativ");
        $("#invert").addClass("positiv");
      }
    });
...
//$('.accordion-section-content:not(.open) input.fontdemo').css("margin-top", "" +-(size/2) +"px");  // PROBLEM
//$('.accordion-section-content.open input.fontdemo').css("margin-top", "" +((15)) +"px");           // PROBLEM
...
//      .animate({ "margin-top":  +-(hh/2)+"px" }, "fast" );
      ...
      $(elem).removeClass('active');
      $('.accordion '+ currentAttrValue).addClass('zu');
      $('.accordion '+ currentAttrValue).removeClass('auf');
      $('.accordion-section-content.zu input.fontdemo').css("margin-top", "" +-(hh/2)+"px");           // NEU
...
//      .animate({ "margin-top":  +margintop+"px" }, "fast" );
      ...
      $(elem).addClass('active');
      $('.accordion '+ currentAttrValue).addClass('auf');
      $('.accordion '+ currentAttrValue).removeClass('zu');
      $('.accordion-section-content.auf input.fontdemo').css("margin-top", "" +margintop+"px");           // NEU
}
